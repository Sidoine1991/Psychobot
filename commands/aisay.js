...
async function callLLM(baseUrl, apiKey, model, messages, maxTokens = 512) {
    const resp = await axios.post(`${baseUrl}/chat/completions`, {
        model, messages, temperature: 0.7, max_tokens: maxTokens, stream: false
    }, {
...
module.exports = {
    ...
    run: async ({ sock, msg, args, replyWithTag }) => {
        ...
        try {
            ...
            if (!reply) return replyWithTag(sock, msg.key.remoteJid, msg, "❌ IA indisponible.");
            ...
            try {
                ...
                await sock.sendMessage(msg.key.remoteJid, { audio: { url: audioPath }, mimetype: 'audio/ogg; codecs=opus', ptt: true }, { quoted: msg });
    ...
    onMessage: async (sock, msg, text) => {
        ...
        if (hasTrigger) {
            try {
                ...
                fs.unlinkSync(audioPath);
                return true;
            } catch (e) { console.error("[AiSay Passive]", e.message); }
        }
        return false;
    }
};
