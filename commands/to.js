...
module.exports = {
    name: 'to',
    description: 'Ajouter destinataire',
    ...
    async run({ sock, msg, args }) {
        ...
        try {
            if (args.length === 0) {
                await sock.sendMessage(remoteJid, { text: '❌ Usage: !to <email>' }, { quoted: msg });
                return;
            }
            ...
            const email = args[0];
            session.compose.to.push(email);
            
            await sock.sendMessage(remoteJid, { text: `✅ TO ajouté: ${email}` }, { quoted: msg });
        } catch (error) {
